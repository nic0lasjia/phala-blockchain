...
use frame_support::{fail, ensure, decl_module, decl_storage, decl_event, decl_error, dispatch};
...
const IAS_QUOTE_STATUS_LEVEL_1: &'static [&str] = &[
	"OK",
];
const IAS_QUOTE_STATUS_LEVEL_2: &'static [&str] = &[
	"SW_HARDENING_NEEDED",
];
const IAS_QUOTE_STATUS_LEVEL_3: &'static [&str] = &[
	"CONFIGURATION_NEEDED",
	"CONFIGURATION_AND_SW_HARDENING_NEEDED",
];
const IAS_QUOTE_STATUS_LEVEL_4: &'static [&str] = &[
	"GROUP_OUT_OF_DATE",
];
const IAS_QUOTE_ADVISORY_ID_WHITELIST: &'static [&str] = &[
...
decl_module! {
	pub struct Module<T: Config> for enum Call where origin: T::Origin {
		...
		pub fn register_worker(origin, encoded_runtime_info: Vec<u8>, report: Vec<u8>, signature: Vec<u8>, raw_signing_cert: Vec<u8>) -> dispatch::DispatchResult {
			...
			let now_func = webpki::Time::from_seconds_since_unix_epoch(now);
			...
			let parsed_report_timestamp = chrono::DateTime::parse_from_rfc3339(&raw_report_timestamp);
			let report_timestamp = match parsed_report_timestamp {
				Ok(timestamp) => timestamp.timestamp(),
				Err(_) => fail!(Error::<T>::BadIASReport)
			};
			...
			if let Some(advisory_ids) = parsed_report["advisoryIDs"].as_array() {
				for advisory_id in advisory_ids {
					let advisory_id = match advisory_id.as_str() {
						Some(id) => id,
						None => fail!(Error::<T>::BadIASReport)
					};
...
impl<T: Config> Module<T> {
	...
	fn register_worker_internal(
		stash: &T::AccountId, machine_id: &Vec<u8>, pubkey: &Vec<u8>, worker_features: &Vec<u32>, confidence_level: u8
	) -> Result<(), Error<T>> {
